...
ggplot(ONP_Final) + geom_col(aes(x=(Day_of_Week),y=shares), color='palegreen4')+scale_x_discrete(limits = positions)
```

Graph videos vs shares.
```{r}
ggplot(ONP_Final,aes(num_videos,shares) ) + geom_col(size = 1, alpha = 5, color="palegreen4")+xlim(0,10)+ylim(0,3e+07)
```

Graph of images vs shares.
```{r}
ggplot(ONP_Final, aes(num_imgs,shares)) + geom_col(size = .1, colour = "palegreen4",  )+xlim(0,30)
```

Article channel vs day of week.
```{r}
ggplot(ONP_Final,aes(Article_Channel, shares) ) + geom_col(size =1, alpha = 0.9, color="palegreen4")
...
neural net for funsies 
```{r}
ONP.net <- neuralnet(popularity ~ n_tokens_content+num_imgs+num_videos, ONP_Final, hidden = 4, linear.output = F, threshold = .2)
```

```{r}
plot(ONP.net,rep="best")
```

```{r}
ONP.net$result.matrix
```
