...
```{r}
ggplot(ONP_Final) + geom_col(aes(x=(Day_of_Week),y=shares), color='blue')
```

Graph videos vs shares.
```{r}
ggplot(ONP_Final,aes(num_videos,shares) ) + geom_col(size = 5, alpha = 0.5, color="red")
```

Graph of images vs shares.
```{r}
ggplot(ONP_Final, aes(num_imgs,shares)) + geom_col(size = 3, colour = "blue",  )
```

Article channel vs day of week.
```{r}
ggplot(ONP_Final,aes(Article_Channel, shares) ) + geom_col(size =1, alpha = 0.9, color="red")
```
