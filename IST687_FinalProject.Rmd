...
```{r}
ONP.lm = glm(formula=shares ~ num_videos + num_imgs + n_tokens_content + rate_positive_words+rate_negative_words+Day_of_Week+Article_Channel, data = ONP_Final)
summary(ONP.lm)
```
...
```{r}
ONP.net <- neuralnet(popularity ~ n_tokens_content+num_imgs+num_videos, df_testing, hidden = 4, linear.output = F, threshold = .2)
```
