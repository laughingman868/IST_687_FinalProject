...
set.seed(111)
trainList <- createDataPartition(y=df_Final_1$shares, p=.80,list=F)
trainSet <- df_Final_1[trainList,]
testSet <- df_Final_1[-trainList,]
#fit1 <- lm(shares ~., trainSet)
svm.model <- train(shares~Article_Channel+Day_of_Week, data=trainSet, method="svmRadial", 
...
confusion1 <- confusionMatrix(svmPred, testSet$shares)
confusion1
```
